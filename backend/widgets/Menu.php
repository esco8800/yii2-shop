 ...
 * Меню для AdminLTE
 *
 * Пункт меню может состоять из следующих элементов:
 * - label    наименование пункта меню
 * - icon     название иконки в FontAwesome без префиксов
 * - items    вложеные пункты меню
 * - url      полный URL адрес пункта меню
 * - hides    скрытые пункты URL, влияющие на определение активности пункта меню
 * - visible  переключатель отображение пункта в меню (можно указывать роль или разрешение из RBAC)
 * - active   переключатель активности пункта в меню
 * - encode   переключатель кодирования спецсимволов
 */
class Menu extends \yii\widgets\Menu
{
    ...
    /**
     * @var array названия действия CRUD по умолчанию. Необходимы для правильной подсветки активного родительского пункта,
     * когда находимся в CRUD действии
     */
    public $crudActions = [
        'create', 'update', 'view',
    ];
    ...
    protected function normalizeItems($items, &$active)
    {
        foreach ($items as $i => $item) {
            ...
            if (!empty($this->labelTranslate)) {
                $item['label'] = Yii::t($this->labelTranslate, $item['label']);
            }
    ...
    protected function isItemActive($item)
    {
        if (isset($item['url']) && is_array($item['url']) && isset($item['url'][0])) {
            ...
            $itemBaseRoute = str_replace('/index', '', $route);
            $currentBaseRoute = str_replace('/index', '', $this->route);

            $currentBaseRoute =  str_replace(array_map(function ($value) {
                return ($value[0] === '/' ? $value : '/' . $value);
            }, $this->crudActions), '', $currentBaseRoute);

            if (!empty($item['hides'])) {
                foreach ($item['hides'] as $hide) {
                    $hide = ltrim($hide, '/');
                    $hide = str_replace('/index', '', $hide);
                    if ($hide == $currentBaseRoute) {
                        return true;
                    }
                }
            }

            // Сравниваем два базовых маршрута
            if ($route !== $this->route && $itemBaseRoute != $currentBaseRoute && $itemBaseRoute !== $this->noDefaultRoute && $itemBaseRoute !== $this->noDefaultAction) {
                return false;
            }
